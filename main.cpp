...
//  main.cpp
//  CellTracking
//  gets rid of shorter sequences
//  gets rid non-moving cells
//  automates output into excel files
//  separates data with blinking lights
//
...
int main(int argc, char** argv){
    ...
    String videoName = "11-30-21 constant light2.avi";
    // set the path to the folder with the videos:
    String path = "/Users/Maximus/Documents/3dscope/Videos/";
    // insert the times when the light changes (on/off) here:
    vector<int> blinkTimes = {300};
    ...
    Mat frame;
    if(!capture.isOpened())
        throw "Error when reading" + videoName + ".avi";
    namedWindow("raw",0);
    ...
    // each entry in this vector is an experiment.  Every time the light blinks on/off, a new entry is created
    vector<vector<vector<Point> > > cellsArr;
    // adds the total number of frames in the video
    blinkTimes.push_back(capture.get(CV_CAP_PROP_FRAME_COUNT));
    // prints out the total number of frames in the video
    cout << capture.get(CV_CAP_PROP_FRAME_COUNT) << endl;
    int numFrames = 0;

    // added to the list of coordinates to mark the end of tracking a single cell
    Point2f end(-1, -1);
    ...
    for(int i = 0; i < blinkTimes.size(); i++){
        Sheet* sheet = book->addSheet(("Sheet" + to_string(i)).c_str());
        vector<vector<Point> > cells(numCells);
        while(1)
        {
            if(numFrames == blinkTimes[i]){
                cellsArr.push_back(cells);
                break;
            }
            numFrames++;
            //for display
            ...
            adaptiveThreshold(frame,frame,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY,7,3); // adaptive threshold
        ...
        sheet->writeFormula(2, 8, ("=1.96*STDEV.S(I5:I" + to_string(row) + ")/SQRT(COUNT(I5:I" + to_string(row) + "))").c_str());
