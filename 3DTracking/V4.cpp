...
// get it to work with cut off radii
...
vector<float> radialAveraging(Mat im, int ctrX, int ctrY, int maxRadius){//takes in ORIGINAL BLACK AND WHITE NON-THRESHOLDED cropped image, (x,y) is the center
  ...
  for(int r = 0; r < maxRadius; r++){//for every radius
    for(int y = ctrY-r; y <= ctrY+r; y++){
      ...
      //for debugging
      if(r == 20)
        circle(im, Point(ctrX + offsetX,y),1, Scalar(255),FILLED);
...
int main(int argc, char** argv){
    ...
    for(int thresh = 250; thresh >= 50; thresh -= 3){
      ...
      findContours(image,contours,hierarchy,RETR_LIST,RETR_TREE);//allows only key points to be added
      if(contours.size() > 0)
      ...
      for(int i = 0; i < contours.size() && i < 7; i++)
      {
         ...
         if(approx.size() > 8 && radius*radius*PI/contourArea(contours[i]) < 1.3 && contourArea(contours[i]) > minArea){//airy pattern: inner ring, 15% margin of error, previous values: 8, 1.3
          //cout << "(" << center.x << "," << center.y << "):" << endl;//ANSWERS
             circle(newIm, center, 4, Scalar(125), FILLED);
